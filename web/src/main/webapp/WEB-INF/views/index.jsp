<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<html ng-app="app">
<head>
    <meta charset="utf-8" />
    <title>AngularJS User Registration and Login Example</title>
    <link rel="stylesheet" href="../../lib/bootstrap.min.css" />
    <link href="../../css/app.css" rel="stylesheet" />
</head>
<body>

<!-- 自适应屏幕大小 -->
<div class="jumbotron">
    <div class="text-left text-muted" ><h1>Let's GOOOOOOOOOOO For Chinese Now!</h1></div>
    <div class="container">
        <div class="col-sm-8 col-sm-offset-2">
            <div ng-class=
                         "{ 'alert': flash, 'alert-success': flash.type === 'success', 'alert-danger':
                         flash.type === 'error' }" ng-if="flash" ng-bind="flash.message"></div>
            <div ng-view>
            </div>
        </div>
    </div>
    <br /><br /><br /><br /><br /><br />
    <div class="text-center text-muted">
        <p>
            <a href="">
                Developed by ZhangMinglei(张明磊) Liugaowei(刘高伟) Niechu(聂础)</a>
        </p>
    </div>
    <div class="text-center text-muted">
        <p>
            <a href="http://www.ustc.edu.cn/" class="text-muted">中国科学技术大学</a>
        </p>
    </div>
</div>
<!--<div class="jumbotron"> <h2 class="text-center text-muted" >WOW. You have found a secret here,-->
<!--<a href="">click me!</a></h2>-->
<!--</div>-->
<script src="../../lib/jquery-3.1.1.min.js"></script>
<script src="../../lib/angular.min.js"></script>
<script src="../../lib/angular-route.min.js"></script>
<script src="../../lib/angular-cookies.min.js"></script>

<script src="../../scripts/app.js"></script>
<script src="../../scripts/services/authentication.service.js"></script>
<script src="../../scripts/services/flash.service.js"></script>

<!-- Real user service that uses an api -->
<script src="../../scripts/services/user.service.js"></script>

<!-- Fake user service for demo that uses local storage -->
<!--<script src="app-services/user.service.local-storage.js"></script>-->

<script src="../../scripts/controllers/home.controller.js"></script>
<script src="../../scripts/controllers/login.controller.js"></script>
<script src="../../scripts/controllers/register.controller.js"></script>
</body>
</html>